...
Every word now has a blink animation (`images/<animal>/`, generated by
`tools/generate_animal_frames.py` — run with `.venv/bin/python
tools/generate_animal_frames.py`, requires Pillow). Compiled through
Flipper's standard `fap_icon_assets` pipeline, not hand-rolled, and loops
automatically. Art quality is rough on the real screen, especially horse
and giraffe — flagged to revisit and redraw.
